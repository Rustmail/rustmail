...
pub async fn format_change_value(alc: &AuditLogContext<'_>, change: &Change) -> Option<EmbedField> {
    match change {
        ...
        Change::Allow { old, new } => {
            let name = alc
                .translate("audit_log.change.permissions_allow", None)
                .await;
            let old_val = old
        ...
        Change::AutoArchiveDuration { old, new } => {
            let name = alc
                .translate("audit_log.change.auto_archive_duration", None)
                .await;
            let old_val = old
                .map(|v| format!("{:?}", v))
                .unwrap_or_else(|| "None".to_string());
        ...
        Change::DefaultAutoArchiveDuration { old, new } => {
            let name = alc
                .translate("audit_log.change.default_auto_archive", None)
                .await;
            let old_val = old
                .map(|v| format!("{:?}", v))
                .unwrap_or_else(|| "None".to_string());
        ...
        Change::DefaultMessageNotifications { old, new } => {
            let name = alc
                .translate("audit_log.change.default_notifications", None)
                .await;
            let old_val = old
                .map(|v| format!("{:?}", v))
                .unwrap_or_else(|| "None".to_string());
        ...
        Change::Deny { old, new } => {
            let name = alc
                .translate("audit_log.change.permissions_deny", None)
                .await;
            let old_val = old
                .map(|p| p.bits().to_string())
                .unwrap_or_else(|| "0".to_string());
        ...
        Change::Description { old, new } => {
            ...
            let old_val = old.clone().unwrap_or_else(|| "None".to_string());
            let new_val = new.clone().unwrap_or_else(|| "None".to_string());
            let old_truncated = if old_val.len() > 100 {
                format!("{}...", &old_val[..100])
            } else {
                old_val
            };
            let new_truncated = if new_val.len() > 100 {
                format!("{}...", &new_val[..100])
            } else {
                new_val
            };
            Some(EmbedField::new(
                name,
                format!("`{}` → `{}`", old_truncated, new_truncated),
                false,
            ))
        }
        Change::DiscoverySplashHash { old, new } => {
            let name = alc
                .translate("audit_log.change.discovery_splash", None)
                .await;
            let old_val = old
                .as_ref()
                .map(|h| format!("{:?}", h))
        ...
        Change::EnableEmoticons { old, new } => {
            let name = alc
                .translate("audit_log.change.enable_emoticons", None)
                .await;
            let old_val = old
                .map(|v| v.to_string())
                .unwrap_or_else(|| "None".to_string());
        ...
        Change::ExpireBehavior { old, new } => {
            let name = alc
                .translate("audit_log.change.expire_behavior", None)
                .await;
            let old_val = old
                .map(|v| format!("{:?}", v))
                .unwrap_or_else(|| "None".to_string());
        ...
        Change::ExpireGracePeriod { old, new } => {
            let name = alc
                .translate("audit_log.change.expire_grace_period", None)
                .await;
            let old_val = old
                .map(|v| format!("{} days", v))
                .unwrap_or_else(|| "None".to_string());
        ...
        Change::ExplicitContentFilter { old, new } => {
            let name = alc
                .translate("audit_log.change.explicit_content_filter", None)
                .await;
            let old_val = old
                .map(|v| format!("{:?}", v))
                .unwrap_or_else(|| "None".to_string());
        ...
        Change::PermissionOverwrites { .. } => {
            let name = alc
                .translate("audit_log.change.permission_overwrites", None)
                .await;
        ...
        Change::PreferredLocale { old, new } => {
            let name = alc
                .translate("audit_log.change.preferred_locale", None)
                .await;
            let old_val = old.clone().unwrap_or_else(|| "None".to_string());
            let new_val = new.clone().unwrap_or_else(|| "None".to_string());
            Some(EmbedField::new(
        ...
        Change::PruneDeleteDays { old, new } => {
            let name = alc
                .translate("audit_log.change.prune_delete_days", None)
                .await;
            let old_val = old
                .map(|v| format!("{} days", v))
                .unwrap_or_else(|| "None".to_string());
        ...
        Change::PublicUpdatesChannelId { old, new } => {
            let name = alc
                .translate("audit_log.change.public_updates_channel", None)
                .await;
            let old_val = old
                .map(|id| format!("<#{}>", id.get()))
                .unwrap_or_else(|| "None".to_string());
        ...
        Change::VerificationLevel { old, new } => {
            let name = alc
                .translate("audit_log.change.verification_level", None)
                .await;
            let old_val = old
                .map(|v| format!("{:?}", v))
                .unwrap_or_else(|| "None".to_string());
        ...
        Change::SystemChannelFlags { old, new } => {
            let name = alc
                .translate("audit_log.change.system_channel_flags", None)
                .await;
            let old_val = old

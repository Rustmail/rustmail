...
async fn is_in_thread(msg: &Message, pool: &sqlx::SqlitePool) -> bool {
    ...
    get_user_id_from_channel_id(&channel_id, pool).await.is_some()
}
...
async fn move_channel_to_category(ctx: &Context, msg: &Message, category_id: ChannelId) -> Result<serenity::model::channel::GuildChannel, serenity::Error> {
    msg.channel_id.edit(&ctx.http, EditChannel::new().category(category_id)).await
}
...
fn find_best_match_category(target_name: &str, categories: &[(ChannelId, String)]) -> Option<(ChannelId, String)> {
